# The Calliope mini - a reactive system
...
To be responsive, a reactive system needs to be able to do several things at the same time (concurrently), just like you can.  But the Calliope mini only has one CPU for executing your program, which means it can only execute one program instruction at a time. It can, however, execute millions of instructions in a single second. This points the way to a solution.

Think about how a motion picture projector works - it projects only 24 frames per second, yet this is good enough to provide the illusion of fluid motion on the screen. The Calliope mini can execute millions of instructions per second, so it seems quite possible for the device to both to smoothly scroll the number 42 across the LED screen while looking for button presses and counting them.
...
## The Calliope mini scheduler and queuing up subprograms

The Calliope mini’s *scheduler* provides the capability to concurrently execute different code sequences, relieving us of a lot of low-level programming.   In fact, scheduling is so useful that it is a part of every *operating system*!
...
The `forever` loop actually is a function that takes a subprogram (another function) as a parameter. The function uses the `control.inBackground` function of the Calliope mini runtime to queue a `while true` loop for execution by the scheduler. The while loop has two statements. The first statement runs the subprogram represented by the `body` parameter. The second statement passes control to the scheduler (requesting to “sleep” for 20 milliseconds).
...
Let’s go back to the `count button presses` program and revisit its execution based on what we have learned about the Calliope mini scheduler. As detailed before, the program executes three steps to:
...
As a result, you can easily add a new capability to the Calliope mini by just adding a new subprogram. For example, if you want to add a reset feature to the counter program, all you need to do is add a new event handler for a press of button **B** that sets the global variable "count" to zero, as shown below:
